...
import { GameRoom, RoomPlayer, GameResult, ChatMessage, RoomStateResponse } from "@/types";
...
export const gameService = {
  ...
  // Submit answer
  submitAnswer: async (roomCode: string, payload: SubmitAnswerPayload): Promise<any> => {
    const token = typeof window !== "undefined" ? localStorage.getItem("access_token") : null;
  ...
  // Post chat message
  postChatMessage: async (roomCode: string, payload: PostChatPayload): Promise<ChatMessage> => {
    const token = typeof window !== "undefined" ? localStorage.getItem("access_token") : null;
